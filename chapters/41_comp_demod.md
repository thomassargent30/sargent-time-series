...
spectrum. The technique was put to work in economics by {cite:t}`godfrey1965exploratory`, as a step in
estimating bispectra (the nonlinear diagnostic of {doc}`39_nonlinear_representation`), and
by {cite:t}`sargent1968interest`, to study the changing seasonal behavior of
...
the Parzen window (Godfrey 1965, eqs. 2.7–2.8). {cite:t}`sargent1968interest` used $r=2$ with $m=18$ on
...
{cite:t}`sargent1968interest` demodulated monthly interest rates at the twelve-month cycle
...
[`code/ch41_demod_seasonal.py`](https://github.com/thomassargent30/sargent-time-series/blob/main/code/ch41_demod_seasonal.py).
```
...
[`code/ch41_demod_peak.py`](https://github.com/thomassargent30/sargent-time-series/blob/main/code/ch41_demod_peak.py).
```
...
[`code/ch41_demod_cross.py`](https://github.com/thomassargent30/sargent-time-series/blob/main/code/ch41_demod_cross.py).
```

## Application: the changing seasonal in interest rates, 1951–1960

{cite:t}`sargent1968interest` studied monthly rates on eleven money- and capital-market instruments —
...
[`code/ch41_fred_seasonal.py`](https://github.com/thomassargent30/sargent-time-series/blob/main/code/ch41_fred_seasonal.py).
```
...
seasonal is the pattern Sargent traced back to {cite:t}`macaulay1938movements` and read as evidence that the
...
[`code/ch41_fred_cross.py`](https://github.com/thomassargent30/sargent-time-series/blob/main/code/ch41_fred_cross.py).
```
...
```{bibliography}
:labelprefix: CD
:filter: key in {"blackmantukey1959measurement", "godfrey1965exploratory", "macaulay1938movements", "sargent1968interest"}
```

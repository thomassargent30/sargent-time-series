...
$$
\begin{aligned}
a(L) &= (1 - \alpha_1 L)\cdots (1 - \alpha_n L) \\
...
which is the expression obtained by {cite:t}`muth1960optimal`. Thus we have that $x_t$ has the first-order moving average, first-order autoregressive representation
...
The result {eq}`eq-62a` of {doc}`15_prediction_examples` now applies with $\beta \equiv \rho$ and $\lambda_1 \equiv -a$. Thus we have
